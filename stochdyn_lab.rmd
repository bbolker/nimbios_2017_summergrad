...
\end{split}
$$
(The $\Delta t$ isn't really necessary - we will $\Delta t=1$ below - but it is sometimes more convenient to have it in the equations than to rescale $\gamma$ and $\beta$ to explore the dynamical consequences of discrete time.)
...
An R implementation of the $t \to t+1$ mapping function: this uses the "magic" function `with()` [so we can use the names associated with the parameter and state vectors as labels] and returns the results as a list containing a vector [to make `deSolve` happy]. This version has only process error.
...
SIRdsim1 <- function(t,y,params) {
   g <- with(as.list(c(y,params)), {
      ...
      recover <- rbinom(1,size=I,prob=gamma)
      c(S=S-inf,
        I=I+inf-recover,
        R=R+recover,
        incidence=inf)
...
library("deSolve")
m1 <- ode(t=1:20,
          y=c(S=100,I=1,R=0,incidence=0),
          func=SIRdsim1,
          parms=c(beta=0.04,gamma=0.5),
          method="iteration")
```
...
* Add observation error to incidence (`inf`) by making the observed value a binomial deviate (in R, `rbinom(1,size=inf,prob=rptprob)`): call the function `SIRdsim_obsproc`.
