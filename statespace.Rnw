...
\begin{description}
\item[Gibbs sampling] exploit conditioning, sample parameters one at a time
...
\item interfaces from R (\code{R2jags} package) or MATLAB (\url{https://github.com/msteyvers/matjags}).
\end{itemize}
\end{frame}
...
\item BUGS: \verb+~+ is ``distributed as'' (``stochastic node''),
\verb+<-+ is assignment (``logical node'')
...
<<jagsens,echo=FALSE,fig.width=8,fig.height=7>>=
...
\section{Other approaches to nonlinear dynamical fitting}
\subsection{}
\begin{frame}
