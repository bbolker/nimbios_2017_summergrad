...
\begin{itemize}
\item reparameterize
\item try Hamiltonian MC \cite{carpenter_stan_2016}
...
\begin{itemize}
\item new toolboxes: NIMBLE \cite{li_fitting_2017,de_valpine_programming_2017}, PyMC
\item Hamiltonian Monte Carlo: Stan \cite{carpenter_stan_2016}
\item variational Bayes, expectation-propagation \ldots \cite{gelman_bayesian_2013}
\end{itemize}
\end{frame}
