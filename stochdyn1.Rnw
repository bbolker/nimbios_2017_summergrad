...
(mostly) \emph{phenomenological} and \emph{linear}
\end{frame}
...
\small
\begin{tabular}{cllll}  
  \textbf{Space} & \textbf{Time} & \textbf{Populations} &
    \textbf{Random} & \textbf{Model} \\
    \hline  
